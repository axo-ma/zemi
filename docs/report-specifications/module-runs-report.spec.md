...
List all started runs across samples in execution order using Run, Sample, Status, and Duration columns. Each Run links to its individual Run Report under `runs/`. Each Sample links to its Sample Report under `samples/`. Resolve all paths from actual report references.
